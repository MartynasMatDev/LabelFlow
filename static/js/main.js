...
(function () {
  ...
  function buildQueue(files) {
    ...
    files.forEach(f => {
      ...
      st.className = 'queue-status'; st.dataset.fkey = k;
      st.textContent = valid ? '○' : '⚠';
      st.title = valid ? 'Queued' : (!ALLOWED.has(e) ? 'Invalid type' : '>20 MB');

      const nm = document.createElement('span');
      nm.className = 'queue-name'; nm.textContent = f.name;

      const sz = document.createElement('span');
      sz.className = 'queue-size'; sz.textContent = fmt(f.size);

      const rm = document.createElement('button');
      rm.type = 'button'; rm.textContent = '✕'; rm.title = 'Remove';
      rm.style.cssText = 'background:none;border:none;color:var(--text-muted);cursor:pointer;font-size:12px;padding:0 2px;flex-shrink:0;transition:color .1s;';
  ...
  inputFolder.addEventListener('change', () => {
    const flat = [...inputFolder.files].filter(f => f.webkitRelativePath.split('/').length === 2);
